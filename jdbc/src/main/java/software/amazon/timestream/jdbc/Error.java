...
enum Error {
   ...
   */
  static String lookup(final Error key, final Object... formatArgs) {
  ...
  /**
   * Create {@link SQLException} of error and log the message with a {@link Logger}.
   *
   * @param logger     The {@link Logger} contains log info.
   * @param key        Resource key for bundle provided to constructor.
   * @param formatArgs Any additional arguments to format the resource string with.
  ...
  static SQLException createSQLException(
      ...
      final Error key,
      final Object... formatArgs) {
    return createSQLException(logger, null, key, formatArgs);
  }
   ...
   */
  static SQLFeatureNotSupportedException createSQLFeatureNotSupportedException(
      final Logger logger,
      final Error key,
      final Object... formatArgs) {
    final String error = lookup(key, formatArgs);
